<template>
<div id="test2App" class="testApp viewApp">
	        <div class="container-fluid">
	        	<div class="row">
		        	<p class="test-buttons">
	        	...
	        	<div class="row">
		        	<div class="test-filters">
		        		<div class="form-group filter-search pull-right">
		        			<input v-model="searchQuery" class="form-control input-sm" type="search" placeholder="Search by...">
		        		</div>
	        			...
	        			<div class="tg-selectable-rows">
	        				<div class="table-responsive">
					        	<table class="table" id="table"> 
						        	...
						        	<tbody>
						        		<tr v-for="test in filteredTests">
							        		<td>
												<div class="checkbox selectState">
	        	...
	        	<div class="editorContainer" v-if="seen" :class="{inEdit:editing}">
			        ...
			        <div class="form-group">
			         ...
			         <div class="form-group">
			            ...
			            <input type="text" name="categoryName" id="categoryName" class="form-control" v-model="categoryName"/>
			        </div>
...
<script>
  ...
  import vPagination from '../../app/components/v-pagination.vue';
  
  const config = {
		rootURL: "http://localhost:8080/MyMavenWebTest/rest/test2",	
		formToJSON: function(ele1, ele2, ele3, ele4){
  ...
  export default {
	...
	computed: {
	    ...
	    filteredTests: function () {
		    ...
		    return self.tests.filter(function (test) {
		      return test.testId.indexOf(self.searchQuery) !== -1
		      ||test.testName.indexOf(self.searchQuery) !== -1
		      ||test.testDes.indexOf(self.searchQuery) !== -1
		      ||test.categoryName.indexOf(self.searchQuery) !== -1
		    });
	  	},
	  	orderedUsers: function () {
	  		var self = this;
    		return _.orderBy(self.tests, self.orderQuery);
  		},
  		totalCount: function() {
  			return self.tests.length;
  		}
	},
	filters: {
	...
	methods: {
		...
		add: function(){
			...
			var cat = self.categoryName && self.categoryName.trim();
			if (!name) {
				return;
			}
			...
			$.ajax({
				...
				url: config.rootURL,
				dataType: "json",
				data: config.formToJSON(newId,newName,newDes,newCat),
				success: function(data, textStatus, jqXHR){
		...
		edit: function(test){
			...
			self.categoryName =  test.categoryName;
			self.editing = true;
		},
		update: function(id){
			var self = this;
			...
			$.ajax({
				...
				url: config.rootURL,
				dataType: "json",
				data: config.formToJSON(editId,editName,editDes,editCat),
				success: function(data, textStatus, jqXHR){
